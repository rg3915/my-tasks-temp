...
def timedelta_to_string(value, format='%H:%M:%S'):
    ...
    if value.microseconds:
        ...
        value_with_microseconds = timedelta(
            hours=hours, minutes=minutes, seconds=seconds, microseconds=value.microseconds
        )  # noqa E501
        return datetime.strftime(datetime.strptime(str(value_with_microseconds), '%H:%M:%S.%f'), format_with_seconds)[
            :-3
        ]  # noqa E501
